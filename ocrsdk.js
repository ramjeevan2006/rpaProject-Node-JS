...
function ProcessingSettings() {
	// this.language = "English";	// Recognition language or comma-separated list of languages.
	// this.exportFormat = "txt";	// Output format. One of: txt, rtf, docx, xlsx, pptx, pdfSearchable, pdfTextAndImages, xml.
	this.language = '';	// Recognition language or comma-separated list of languages.
	this.exportFormat = '';	// Output format. One of: txt, rtf, docx, xlsx, pptx, pdfSearchable, pdfTextAndImages, xml.
	this.customOptions = '';	// Other custom options passed to RESTful call, like 'profile=documentArchiving'
	this.region = '';
}
...
ocrsdk.prototype.processImage = function(filePath, settings, userCallback) {
	...
	var req = this._createTaskRequest('POST', '/processTextField' + urlOptions, userCallback);
...
/**
 * Process fields of the document.
 * 
 * @param {string} taskId 	Task identifier as returned in taskData.id.
 * @param {function(error, taskData)} callback 	The callback function.
 */
ocrsdk.prototype.processFields = function(taskId, outputFilePath, userCallback) {
	...
	function waitFunction() {
		recognizer.getTaskStatus(taskId,
			function(error, taskData) {
				...
				console.log("\nTask status is " + taskData.status);
 ...
 */
ocrsdk.prototype.downloadResult = function(resultUrl, outputFilePath, userCallback) {
	console.log('\n** ResultUrl=', resultUrl);
	var file = fs.createWriteStream(outputFilePath);
	...
	parsed = {
		// host: 'i3lwebapp1.itcinfotech.com',
		// hostname: 'i3lwebapp1.itcinfotech.com',
		host: '10.6.13.87',
		port: 8080,
		path: resultUrl,
		// auth: this.appId + ":" + this.password,
		// protocol: 'http:',
		// headers: { 'Host': 'windows.net', 'Proxy-Authorization': 'Basic aXRjaW5mb3RlY2jCsDE0OlMwbGFyMTUh' }
	};
	// console.log('** Result Options=', parsed);
	var req = http.request(parsed, function(response) {
		var data = '';
		...
		response.on('end', function() {
			...
			parser.parseString(data, function(err, objResult) {
				...
				// console.log('\nRESP=' + JSON.stringify(objResult, null, 1));
				file.write(JSON.stringify(objResult, null, 1));
			});
			file.end();
			userCallback(null);
		});
...
ocrsdk.prototype._createTaskRequest = function(method, urlPath, taskDataCallback) {
	...
	var apiPath = this.serverUrl + urlPath;
	...
	/*
	var auth1 = 'aXRjaW5mb3RlY2jCsDE0OlMwbGFyMTUh';
	var auth2 = 'aXRjaW5mb3RlY2glNUMyNjAxNDpTMGxhcjE1IQ==';
	requestOptions.headers['Host'] = requestOptions.host;
	requestOptions.headers['Proxy-Authorization'] = 'Basic aXRjaW5mb3RlY2jCsDE0OlMwbGFyMTUh';
	requestOptions.host = 'i3lwebapp1.itcinfotech.com';
	requestOptions.hostname = 'i3lwebapp1.itcinfotech.com';
	requestOptions.port = 8080;
	requestOptions.path = apiPath;
	*/
	// END CUSTOM OPTIONS
